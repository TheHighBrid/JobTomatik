...
test('option styles always set an explicit readable background and text color', () => {
  for (const state of [
    {},
    { focus: true },
    { selected: true },
...
test('HighContrastSelect uses Headless UI rather than native option elements', () => {
...
test('AnswerPolicyVault uses the high contrast control for policy selectors', () => {
  ...
  assert.equal(vaultSource.includes('aria-label="Policy"'), true)
  assert.equal(vaultSource.includes('aria-label="Scope"'), true)
...
test('native selects retain dark high-contrast CSS as a secondary defense', () => {
