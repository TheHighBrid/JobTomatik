...
export function BrandMark({ className = '', decorative = false, title = 'JobTomatik' }) {
  ...
  const shellId = `${uid}-jt-shell`
  const blueId = `${uid}-jt-blue`
  const goldId = `${uid}-jt-gold`
  const shadowId = `${uid}-jt-shadow`
  ...
  return (
    ...
    >
      <defs>
        <linearGradient id={shellId} x1="10" y1="8" x2="98" y2="102" gradientUnits="userSpaceOnUse">
          <stop stopColor="#162A4A" />
          <stop offset="1" stopColor="#07111F" />
        </linearGradient>
        <linearGradient id={blueId} x1="41" y1="43" x2="66" y2="87" gradientUnits="userSpaceOnUse">
          <stop stopColor="#78B3FF" />
          <stop offset="1" stopColor="#2F6BFF" />
        </linearGradient>
        <linearGradient id={goldId} x1="74" y1="15" x2="97" y2="40" gradientUnits="userSpaceOnUse">
      ...
      <rect x="3" y="3" width="102" height="102" rx="26" fill={`url(#${shellId})`} />
      <rect x="4" y="4" width="100" height="100" rx="25" fill="none" stroke="#284A78" strokeOpacity=".55" />

      <g filter={`url(#${shadowId})`}>
        <path d="M18 42h24" stroke={`url(#${blueId})`} strokeWidth="6" strokeLinecap="round" />
        <path d="M13 53h24" stroke={`url(#${blueId})`} strokeWidth="6" strokeLinecap="round" opacity=".9" />
        ...
        <path d="m52.5 67 7.5 17-7.5 8-7.5-8 7.5-17Z" fill={`url(#${blueId})`} />
