"""Synthetic-only helpers for supervised SmartRecruiters live certification."""
...
SYNTHETIC_TEXT_RESPONSE = (
    "Synthetic SmartRecruiters certification response. This public form is being "
    "tested in dry-run mode and will not be submitted."
)


async def inspect_smartrecruiters_application_dom(surface: Any) -> Dict[str, Any]:
    inventory = await _inspect_application_dom(surface)
    inventory["platform"] = "smartrecruiters"
...
def build_synthetic_profile(dom_inventory: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for index, policy in enumerate(profile.get("answer_policies") or [], start=1):
        ...
        value["scope_value"] = "smartrecruiters.com"
        ...
        if canonical.startswith("custom.lever_synthetic_"):
            value["canonical_key"] = canonical.replace(
                "custom.lever_synthetic_", "custom.smartrecruiters_synthetic_", 1
            )
    ...
    profile["synthetic_platform"] = "smartrecruiters"
    return profile
...
async def build_synthetic_profile_for_page(
    ...
    inventory = await inspect_smartrecruiters_application_dom(surface)
...
def write_synthetic_resume(path: str) -> str:
    ...
    draw.multiline_text(
        (48, 48),
        "AVERY CERTIFICATION\n\n"
        "Synthetic SmartRecruiters ATS Certification Candidate\n\n"
    ...
    if not target.exists() or target.stat().st_size < 100:
        raise RuntimeError(
            "Synthetic SmartRecruiters certification resume could not be generated."
        )
    return str(target)
