...
celery_app = Celery(
    ...
    include=[
        "app.tasks.scraping", "app.tasks.discovery", "app.tasks.applications",
        "app.tasks.handoffs", "app.tasks.operator_assisted", "app.tasks.unattended",
        "app.tasks.followup", "app.tasks.operations", "app.tasks.runtime", "app.tasks.shadow_runs",
    ],
)
...
celery_app.conf.update(
    ...
    beat_schedule={
        "check-followups-every-hour": {"task": "app.tasks.followup.send_pending_followups", "schedule": crontab(minute=0)},
        "continuous-job-discovery-hourly": {"task": "app.tasks.discovery.run_continuous_discovery", "schedule": crontab(minute=12)},
        "recover-stale-followup-deliveries": {"task": "app.tasks.followup.recover_stale_followup_deliveries", "schedule": crontab(minute="7,22,37,52")},
        "recover-stale-application-attempts": {"task": "app.tasks.operations.recover_stale_application_attempts", "schedule": crontab(minute="5,20,35,50")},
        "reconcile-human-final-submissions": {"task": "app.tasks.operations.reconcile_operator_submissions", "schedule": crontab(minute="*")},
        "recover-stalled-shadow-campaigns": {"task": "app.tasks.shadow_runs.recover_stalled_shadow_sessions", "schedule": crontab(minute="11,26,41,56")},
        "refresh-adapter-health-alerts-hourly": {"task": "app.tasks.operations.refresh_adapter_health_alerts", "schedule": crontab(minute=15)},
        "refresh-job-scores-daily": {"task": "app.tasks.scraping.refresh_all_scores", "schedule": crontab(hour=3, minute=0)},
        "daily-auto-search": {"task": "app.tasks.scraping.daily_auto_search_all", "schedule": crontab(hour="*/6", minute=0)},
    },
)
...
def install_worker_task_integrations(**_kwargs):
    ...
    from app.services.application_target_handoff import install_application_target_handoff_task_persistence
    from app.services.application_target_task_integration import install_application_target_task_integration
    ...
    from app.services.operator_assisted_live_pilot_hardening import install_operator_assisted_live_pilot_hardening
    from app.services.operator_autonomy_control_integration import install_operator_autonomy_control
    from app.services.supervised_submission_integration import install_supervised_submission_task_gate
