...
@pytest.mark.asyncio
async def test_thanks_for_applying_is_strong_confirmation():
...
@pytest.mark.asyncio
async def test_weak_phrase_requires_structural_change_and_submit_control_absence():
    adapter = _TestLeverAdapter(
        after_fingerprint="after",
        submit_control_present=False,
    )
    ...
    assert evidence[0].metadata["confirmation_basis"] == (
        "weak_phrase_plus_structural_change"
    )


@pytest.mark.asyncio
async def test_weak_phrase_does_not_certify_unchanged_submit_form():
    adapter = _TestLeverAdapter(
    ...
    assert item.metadata["fingerprint_changed"] is False
...
@pytest.mark.asyncio
async def test_visible_confirmation_selector_remains_sufficient():
    adapter = _TestLeverAdapter(
        after_fingerprint="after",
        submit_control_present=False,
    )
