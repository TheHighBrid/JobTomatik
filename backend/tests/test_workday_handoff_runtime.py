...
@pytest.mark.asyncio
async def test_workday_retained_browser_survives_disconnect_and_resumes_same_form(tmp_path):
    from playwright.async_api import async_playwright

    resume = tmp_path / "resume.pdf"
    resume.write_bytes(b"%PDF-1.4\nWorkday resumable handoff certification")
    ...
    try:
        try:
            ...
            if os.getenv("REQUIRE_BROWSER_TESTS") == "1":
                pytest.fail(
                    f"Retainable Chromium is required for Workday handoff certification: {exc}"
                )
            pytest.skip("Retainable Chromium is not available")

        await runtime.page.set_content(
            f"""
            <form id="workday-application" action="{WORKDAY_ACTION}">
        ...
        await runtime.page.locator("#first").fill("Avery")
        await runtime.page.locator("#last").fill("Certification")
        ...
        snapshot = await runtime.capture_snapshot(
            metadata={"dry_run": True, "adapter": "workday"}
        )

        session = ManualHandoffSession(
            public_id="workday-browser-runtime-test",
            application_id=1,
            manual_review_id=1,
            user_id=1,
        ...
        try:
            assert await retained_page.locator("#first").input_value() == "Avery"
            assert await retained_page.locator("#last").input_value() == "Certification"
            assert (
                await retained_page.locator("#email").input_value()
                == "avery.certification@example.test"
            )
            assert (
                await retained_page.locator("#resume").evaluate("(el) => el.files[0].name")
                == "resume.pdf"
            )
        ...
        third_playwright, _, _, final_page = await _connect_local_cdp(session)
        try:
            assert await final_page.evaluate("window.submitAttempts || 0") == 0
        finally:
            await _disconnect(third_playwright)
    finally:
