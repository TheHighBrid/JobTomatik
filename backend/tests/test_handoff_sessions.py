...
def make_records(db, reason=ManualReviewReason.captcha_detected.value):
    user = User(
...
def test_issue_is_idempotent_and_never_uses_legacy_plaintext_token(db_session):
    _, _, application, review = make_records(db_session)
...
def test_claim_consumes_resume_token_and_issues_short_lived_lease(db_session):
    user, _, application, review = make_records(db_session)
    issued = issue_handoff_session(db_session, application, review, browser_provider="local_cdp")
    db_session.commit()
...
def test_invalid_token_and_wrong_owner_are_rejected(db_session):
    user, _, application, review = make_records(db_session)
    issued = issue_handoff_session(db_session, application, review, browser_provider="local_cdp")
    db_session.commit()
...
def test_heartbeat_ready_and_resume_state_machine_is_retry_safe(db_session):
    user, _, application, review = make_records(db_session, ManualReviewReason.mfa_required.value)
    issued = issue_handoff_session(db_session, application, review, browser_provider="local_cdp")
    claimed = claim_handoff_session(
        db_session,
        issued.session,
    ...
    complete_handoff_resume(db_session, issued.session, result={"ready_to_submit": True})
    complete_handoff_resume(db_session, issued.session, result={"ready_to_submit": True})
    assert issued.session.status == HandoffSessionStatus.completed.value


def test_expired_session_cannot_be_claimed(db_session):
    user, _, application, review = make_records(db_session)
    issued = issue_handoff_session(db_session, application, review, browser_provider="local_cdp")
...
def test_non_resumable_review_reason_is_rejected(db_session):
    _, _, application, review = make_records(db_session, ManualReviewReason.ambiguous_question.value)
    with pytest.raises(ValueError):
        issue_handoff_session(db_session, application, review, browser_provider="local_cdp")
