...
def test_ingestion_appends_server_built_record_and_refreshes_readiness(db_session, tmp_path):
    ...
    assert result["record"]["approval_reference"]
    assert result["record"]["review_reference"]
    assert result["ledger_record_count"] == 1
...
def test_conflicting_replay_fails_closed_and_preserves_existing_ledger(db_session, tmp_path):
    user, job, application = _confirmed_fixture(db_session)
    paths = _paths(tmp_path)
    first = ingest_confirmed_supervised_application(db_session, application, user, job, **paths)
    db_session.commit()
...
def test_readiness_reader_uses_shared_lock_and_returns_digest(db_session, tmp_path):
    user, job, application = _confirmed_fixture(db_session)
    paths = _paths(tmp_path)
    ingest_confirmed_supervised_application(db_session, application, user, job, **paths)
    db_session.commit()
