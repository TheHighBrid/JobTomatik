...
def _session(*, live_snapshot_checkpointed: bool = False) -> ManualHandoffSession:
    return ManualHandoffSession(
        ...
        handoff_metadata={
            ...
            "supervised_target": {"adapter": "lever", "adapter_version": "1.1.0"},
        },
    )
...
@pytest.mark.parametrize(
    ...
    [
        (SimpleNamespace(global_kill_switch=True, autopilot_enabled=False, disabled_platforms=""), SimpleNamespace(allow_real_application_submit=False, lever_supervised_pilot_enabled=False), "global_kill_switch_active"),
        (SimpleNamespace(global_kill_switch=False, autopilot_enabled=True, disabled_platforms=""), SimpleNamespace(allow_real_application_submit=False, lever_supervised_pilot_enabled=False), "operator_assisted_requires_autopilot_disabled"),
        (SimpleNamespace(global_kill_switch=False, autopilot_enabled=False, disabled_platforms=""), SimpleNamespace(allow_real_application_submit=True, lever_supervised_pilot_enabled=False), "operator_assisted_requires_global_submit_disabled"),
        (SimpleNamespace(global_kill_switch=False, autopilot_enabled=False, disabled_platforms=""), SimpleNamespace(allow_real_application_submit=False, lever_supervised_pilot_enabled=True), "operator_assisted_requires_platform_pilot_disabled"),
        (SimpleNamespace(global_kill_switch=False, autopilot_enabled=False, disabled_platforms="lever"), SimpleNamespace(allow_real_application_submit=False, lever_supervised_pilot_enabled=False), "platform_disabled"),
    ],
)
def test_operator_final_action_runtime_blockers_fail_closed(monkeypatch, operations, core, expected):
    ...
    assert expected in integration._operator_final_action_blockers(LEVER_URL)
...
async def test_global_kill_switch_blocks_before_browser_connection(monkeypatch):
    ...
    monkeypatch.setattr(integration, "get_operations_settings", lambda: SimpleNamespace(global_kill_switch=True, autopilot_enabled=False, disabled_platforms=""))
    monkeypatch.setattr(integration, "get_settings", lambda: SimpleNamespace(allow_real_application_submit=False, lever_supervised_pilot_enabled=False))
    connected = []
    ...
    with pytest.raises(browser_handoff.BrowserHandoffError, match="global_kill_switch_active"):
        await browser_handoff.perform_handoff_action(_session(), action="operator_submit")
    assert connected == []
...
async def test_runtime_gate_drift_after_checkpoint_blocks_before_click(monkeypatch):
    ...
    class Control:
        ...
        async def click(self): events.append("click")
    ...
    class Adapter:
        name = "lever"
        version = "1.1.0"
        async def resolve_surface(self, _page): return object()
        async def step_fingerprint(self, _surface): return "lever-step-before"
        async def find_submit_button(self, _surface): return control
        async def extract_validation_errors(self, _surface): return []
        async def detect_confirmation(self, _surface, **_kwargs): return []

    async def connect(_session): return object(), None, None, Page()
    async def verify(_page, _session, **_kwargs): return {"verified": True, "blockers": []}
    async def detect(_page, _url): return Adapter()
    async def disconnect(_playwright): return None
    async def page_fingerprint(_page): return "fresh-live-before"
    ...
    monkeypatch.setattr(browser_handoff, "_session_supervised_target", lambda _session: {"adapter": "lever", "adapter_version": "1.1.0"})
    ...
    def checkpoint(_session, *, current_url, current_fingerprint):
        ...
        _session.handoff_metadata = {**dict(_session.handoff_metadata or {}), "operator_submit_live_snapshot_checkpointed": True, "operator_submit_pre_submit_url": current_url, "operator_submit_pre_submit_fingerprint": current_fingerprint}

    monkeypatch.setattr(integration, "_checkpoint_fresh_live_snapshot", checkpoint)
    with pytest.raises(browser_handoff.BrowserHandoffError, match="global_kill_switch_active"):
        await browser_handoff.perform_handoff_action(session, action="operator_submit")
    assert events == ["checkpoint"]
    assert blocker_calls == [1, 2, 3]


@pytest.mark.asyncio
async def test_target_verified_retained_human_confirmation_does_not_require_click_checkpoint(monkeypatch):
    integration.install_operator_assisted_handoff_integration()
    ...
    async def generic_transition(_session):
        return browser_handoff.BrowserVerification(
            ...
            evidence={
                ...
                "confirmation_evidence": [{"evidence_type": "success_banner", "is_sufficient": True, "confirmation_text": "thank you for applying"}],
    ...
    result = await browser_handoff.verify_browser_handoff_completion(_session(live_snapshot_checkpointed=False))
    ...
    assert result.evidence["retained_human_confirmation"] is True
