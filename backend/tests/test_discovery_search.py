...
async def test_explicit_public_ats_results_survive_broad_board_global_limit(monkeypatch):
    """High-volume broad discovery must not starve an explicitly configured ATS path."""
    ...
    assert any(item["kind"] == "public_ats" and item["result_count"] == 1 for item in observed["source_diagnostics"])
