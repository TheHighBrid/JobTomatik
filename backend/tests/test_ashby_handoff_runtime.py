...
@pytest.mark.asyncio
async def test_ashby_retained_browser_survives_disconnect_and_resumes_same_form(tmp_path):
    from playwright.async_api import async_playwright

    resume = tmp_path / "resume.pdf"
    resume.write_bytes(b"%PDF-1.4\nAshby resumable handoff certification")
    ...
    try:
        try:
            ...
            if os.getenv("REQUIRE_BROWSER_TESTS") == "1":
                pytest.fail(f"Retainable Chromium is required for Ashby handoff certification: {exc}")
        ...
        snapshot = await runtime.capture_snapshot(metadata={"dry_run": True, "adapter": "ashby"})

        session = ManualHandoffSession(
            public_id="ashby-browser-runtime-test",
            application_id=1,
            manual_review_id=1,
            user_id=1,
        ...
        try:
            ...
            assert await retained_page.locator("#email").input_value() == "avery.certification@example.test"
            assert await retained_page.locator("#resume").evaluate("(el) => el.files[0].name") == "resume.pdf"
        ...
        assert result["ats_adapter"] == "ashby"
        assert not any(
