...
@pytest.mark.asyncio
async def test_native_identity_exhausts_http_retry_budget_without_recursive_cdp(monkeypatch):
    calls = 0
    sleeps: list[float] = []
...
@pytest.mark.asyncio
async def test_native_identity_still_fails_closed_when_http_fails(monkeypatch):
    async def fake_http(_endpoint):
        request = httpx.Request("GET", "http://127.0.0.1:9223/json/version")
