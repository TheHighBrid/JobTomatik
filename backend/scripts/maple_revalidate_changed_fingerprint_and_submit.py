...
from app.services import browser_handoff, handoff_integration, handoff_session
...
AUTH = f"SUBMIT {EMPLOYER} | {ROLE} | {URL}"
...
from app.schemas.supervised_submission import OperatorAssistedApprovalCreate, OperatorAssistedFinalSubmitRequest


async def inspect_live(session: ManualHandoffSession) -> dict:
    playwright, _, _, page = await browser_handoff._connect_local_cdp(session)
    try:
        url = str(page.url or "")
        verification = await browser_handoff._verify_session_target(page, session)
        browser_handoff._require_verified_session_target(verification)
        ...
        if adapter.name != "lever":
            raise RuntimeError(f"STOP_ADAPTER_CHANGED actual={adapter.name}")
        if expected_version and adapter.version != expected_version:
            raise RuntimeError(
                f"STOP_ADAPTER_VERSION_CHANGED expected={expected_version} actual={adapter.version}"
            )

        surface = await adapter.resolve_surface(page)
        submit = await adapter.find_submit_button(surface)
        if submit is None:
            raise RuntimeError("STOP_SUBMIT_CONTROL_MISSING")
        if not await submit.is_visible() or not await submit.is_enabled():
            raise RuntimeError("STOP_SUBMIT_CONTROL_NOT_VISIBLE_ENABLED")

        validation_errors = await adapter.extract_validation_errors(surface)
        if validation_errors:
            raise RuntimeError(f"STOP_VALIDATION_ERRORS_PRESENT count={len(validation_errors)}")

        required = await page.evaluate(
            """() => {
              const visible = (el) => !!(el.offsetWidth || el.offsetHeight || el.getClientRects().length);
              const controls = Array.from(document.querySelectorAll(
                'input:not([type=hidden]),textarea,select,[role=combobox][aria-required=true],[role=radio][aria-required=true],[role=checkbox][aria-required=true]'
              )).filter(el => visible(el) && (el.required || el.getAttribute('aria-required') === 'true'));
              const empty = [];
              ...
              for (const el of controls) {
                ...
                if (type === 'radio') {
                  ...
                  ok = String(el.value || el.textContent || el.getAttribute('aria-label') || '').trim().length > 0;
                } else {
                  ok = String(el.value || '').trim().length > 0;
                }
                if (!ok) empty.push({tag, type, name, id});
              }
              return {required_count: controls.length, empty};
            }"""
        )
        if required.get("empty"):
            raise RuntimeError(
                "STOP_REQUIRED_FIELDS_EMPTY " + json.dumps(required.get("empty"), sort_keys=True)
            )
        ...
        return {
            ...
            "target_verified": bool(verification.get("verified")),
            "target_blockers": list(verification.get("blockers") or []),
            "adapter": adapter.name,
            "adapter_version": adapter.version,
            "submit_visible_enabled": True,
            "validation_error_count": 0,
            "required_count": int(required.get("required_count") or 0),
...
def run() -> int:
    db = SessionLocal()
    try:
        app = db.query(Application).filter(Application.id == APP_ID).with_for_update().one()
        ...
        old = db.query(ManualHandoffSession).filter(
        ...
        review = db.query(ManualReviewTask).filter(
            ManualReviewTask.id == old.manual_review_id
        ).with_for_update().one()
        ...
        if review.status not in {ManualReviewStatus.open.value, ManualReviewStatus.in_progress.value}:
            raise RuntimeError(f"STOP_REVIEW_NOT_ACTIVE status={review.status}")

        first = asyncio.run(inspect_live(old))
        if first["url"] != URL:
            raise RuntimeError(f"STOP_LIVE_URL_CHANGED actual={first['url']}")

        emit("MAPLE_CHANGED_FINGERPRINT_REVALIDATED", **first)
        ...
        issued = handoff_session.issue_handoff_session(
            ...
            current_fingerprint=first["fingerprint"],
        ...
        notification = create_handoff_required_notification(db, app, replacement, issued.session)
        replacement.details = {
        ...
        preflight = build_operator_assisted_preflight(db, app, user, job, target_metadata=target)
        if not preflight.get("ready"):
            raise RuntimeError(
                "STOP_REFRESHED_PREFLIGHT_BLOCKED: " + ", ".join(preflight.get("blockers") or [])
            )
        ...
        db.commit()
        fresh_id = issued.session.public_id
        emit(
            "MAPLE_FRESH_60MIN_HANDOFF_READY",
            handoff_public_id=fresh_id,
        ...
        typed = input().strip()
        if typed != AUTH:
            raise RuntimeError("STOP_AUTHORIZATION_TEXT_MISMATCH")

        db.expire_all()
        fresh = db.query(ManualHandoffSession).filter(
            ManualHandoffSession.public_id == fresh_id
        ).with_for_update().one()
        if fresh.expires_at <= datetime.utcnow():
            raise RuntimeError("STOP_FRESH_HANDOFF_EXPIRED")

        second = asyncio.run(inspect_live(fresh))
        if second["url"] != URL:
            raise RuntimeError(f"STOP_LIVE_URL_CHANGED_AFTER_AUTH actual={second['url']}")
        if second["fingerprint"] != fresh.current_fingerprint:
            raise RuntimeError(
                f"STOP_PAGE_CHANGED_DURING_AUTH expected={fresh.current_fingerprint} actual={second['fingerprint']}"
            )
        emit("MAPLE_AUTHORIZATION_PAGE_REVERIFIED", **second)

        approval = asyncio.run(create_operator_assisted_approval(
            APP_ID,
            OperatorAssistedApprovalCreate(
                ...
                notes="Owner explicitly authorized exact Maple final submit after changed-fingerprint revalidation.",
            ),
            current_user=user,
            db=db,
        ))
        approval_ref = str(approval["reference"])
        emit("MAPLE_EXACT_APPROVAL_BOUND", approval_reference=approval_ref, handoff_public_id=fresh_id)

        authorization = asyncio.run(authorize_operator_final_click(
            APP_ID,
            approval_ref,
            current_user=user,
            db=db,
        ))
        if authorization.get("handoff_public_id") != fresh_id:
        ...
        boot = asyncio.run(bootstrap_handoff(fresh_id, current_user=user, db=db))
        claimed = asyncio.run(claim_handoff(
            fresh_id,
        ...
        try:
            result = asyncio.run(submit_operator_assisted_final_action(
                APP_ID,
                fresh_id,
        ...
        for attempt in range(1, 7):
            try:
                completion = asyncio.run(complete_handoff(
                    fresh_id,
        ...
        for _ in range(45):
            ...
            current_handoff = db.query(ManualHandoffSession).filter(
                ManualHandoffSession.public_id == fresh_id
            ).one()
            final_state = {
            ...
            if current_app.automation_state in {"submitted", "confirmed"} or current_handoff.status == "completed":
                break
            time.sleep(2)
